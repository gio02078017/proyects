...
public class OficinasDeAtencionActivity extends BaseOficinasDeAtencionOEstacionesDeGasActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(datos.getSerializable(Constants.EXTRAS_LOCATION) != null){
            customLocation = (CustomLocation)datos.getSerializable(Constants.EXTRAS_LOCATION);
        }
    }
    ...
    private void loadBinding() {
        ...
        oficinasDeAtencionViewModel.getExpiredToken().observe(this, errorToken -> {
            if(errorToken != null){
                showAlertDialogUnauthorized(oficinasDeAtencionViewModel.getError().getValue().getTitle(), oficinasDeAtencionViewModel.getErrorUnauthorized());
            }
        });
    }
    ...
    protected void startLocation() {
        ...
        getLocationDisplayManager().addLocationChangedListener(locationChangedEvent -> {
            if (!isFirstPointLoaded() || isCallHandLocation() || callHandOrServiceLocation) {
    ...
    private void startActivityDetail(InformationOffice informationOffice){
        if(!controlCallOfficeDetailActivity){
            startActivityForResult(validateTurnAssigned(informationOffice), Constants.DEFAUL_REQUEST_CODE);
