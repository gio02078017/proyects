...
public class ConfigurationFragment extends BaseFragmentDi {
    ...
    private void loadObservable(){
        ...
        configurationNotificationViewModel.getExpiredToken().observe((LifecycleOwner) context , expiredToken -> {
            if(expiredToken != null && expiredToken){
                onConfigurationRecyclerListener.showErrorUnauthorized(configurationNotificationViewModel.getIntTitleError(),
                        configurationNotificationViewModel.getMessageError());
            }
        });
        configurationNotificationViewModel.getIsError().observe((LifecycleOwner) context, error -> {
            if(error != null && error){
                onConfigurationRecyclerListener.showError(configurationNotificationViewModel.getIntTitleError(),
                        configurationNotificationViewModel.getMessageError());
            }
        });
