...
public class CryptLib {
    ...
    private String encryptDecrypt(String _inputText, String _encryptionKey,
        ...
        int len = _encryptionKey.getBytes(Constants.UTF_EIGHT).length; // length of the key	provided

        if (_encryptionKey.getBytes(Constants.UTF_EIGHT).length > _key.length)
            len = _key.length;

        int ivlen = _initVector.getBytes(Constants.UTF_EIGHT).length;

        if (_initVector.getBytes(Constants.UTF_EIGHT).length > _iv.length)
            ivlen = _iv.length;

        System.arraycopy(_encryptionKey.getBytes(Constants.UTF_EIGHT), 0, _key, 0, len);
        System.arraycopy(_initVector.getBytes(Constants.UTF_EIGHT), 0, _iv, 0, ivlen);
        ...
        if (_mode.equals(EncryptMode.ENCRYPT)) {
            ...
            byte[] results = _cx.doFinal(_inputText.getBytes(Constants.UTF_EIGHT)); // Finish
            // multi-part
            // transformation
            // (encryption)
    ...
    public static String SHA256(String text, int length) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        md.update(text.getBytes(Constants.UTF_EIGHT));
