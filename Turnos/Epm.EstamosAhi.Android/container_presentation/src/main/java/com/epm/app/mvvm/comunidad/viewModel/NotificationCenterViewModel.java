...
public class NotificationCenterViewModel extends BaseViewModel implements INotificationCenterViewModel {
    ...
    protected void setErrorListNotification(Throwable e) {
        if ("timeout".equals(e.getMessage())) {
            validateError(new ErrorMessage(R.string.title_appreciated_user, R.string.timeout));
        } else {
            indentifyExceptionNotificationCenter(e);
        }
    }


    private void indentifyExceptionNotificationCenter(Throwable e) {
        if (e instanceof HttpException) {
            ValidateServiceCode.captureServiceErrorCode(((HttpException) e).code());
            validateError(new ErrorMessage(ValidateServiceCode.getTitleError(),ValidateServiceCode.getError()));
        } else {
            validateError(new ErrorMessage(ValidateServiceCode.getTitleError(),ValidateServiceCode.getError()));
        }
    }
