...
public class TypePersonActivity extends BaseActivityWithDI implements TypePersonRecyclerAdapter.OnTypePersonRecyclerListener {
    ...
    private void getInformationBeforeActivity(){
        ...
        if(datosBeforeActivity.getSerializable(Constants.PROCEDURE_INFORMATION) != null){
            ...
            binding.textTypeProcedure.setText(procedureInformation.getNameProcedure());
            loadService();
        }
    }

    private void loadService(){
        typePersonViewModel.fetchTypePerson(new TypePersonRequest(procedureInformation.getIdService(), procedureInformation.getIdProcedure()));
    }

    private void loadBinding() {
        typePersonViewModel.getExpiredToken().observe(this, errorMessage -> showAlertDialogUnauthorized(errorMessage.getTitle(),errorMessage.getMessage()) );
    ...
    public void onItemClick(int position) {
      ...
      //  i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        procedureInformation.setIdMasterDetails(typePersonViewModel.getTypePersonItem(position).getTypePersonId());
