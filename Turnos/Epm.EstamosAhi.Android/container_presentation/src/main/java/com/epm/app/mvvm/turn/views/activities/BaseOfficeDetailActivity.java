...
public class BaseOfficeDetailActivity extends BaseActivityWithDI implements ChooseMapProviderDialogFragment.BottomSheetListener {
    ...
    protected void startMap(MapView mapView, int paddingMap, boolean itDoZoom) {
        ...
        this.mapView.setWrapAroundMode(WrapAroundMode.DISABLED);
    ...
    protected void validatePermissionUbicacion(boolean goValidateGpsIsOn) {
    ...
    protected void itDoHandLocationWithNotDissmissModal(boolean itDoZoom) {
        if (getValidateInternet().isConnected()) {
            this.itDoZoom = itDoZoom;
            validatePermissionUbicacion(false);
        } else {
    ...
    public void downloadImageEasyPointOffice(ImageView imageOffice, ProgressBar progressBar) {
        Glide.with(getApplicationContext())
                ...
                .into(new GlideDrawableImageViewTarget(imageOffice) {
                    ...
                    public void onLoadFailed(Exception e, Drawable errorDrawable) {
                        progressBar.setVisibility(View.GONE);
                        imageOffice.setImageResource(R.drawable.ic_easy_point_office_default);
                    }
                });
    }
    ...
    private void drawWithOutCurrentLocation(InformationOffice informationOffice) {
        ...
        showPointInMap(centerPoint, R.mipmap.ic_chosen_office, 18, 45);
        if (itDoZoom) {
    ...
    private void drawWithCurrentLocation(InformationOffice informationOffice) {
        ...
        if (itDoZoom) {
            paintViewPointGeometryAsync(initialEnvelope, paddingMap);
            itDoZoom = false;
        }
    }
    ...
    public void setItDoZoom(boolean itDoZoom) {
        this.itDoZoom = itDoZoom;
    }
    ...
    public AlertDialog getOfficeDetailDialog() {
        return officeDetailDialog;
    }
    ...
    public void showAlertDialogUnauthorized(final int title, final int message, final int positive) {
        runOnUiThread(() -> getCustomAlertDialog().showAlertDialog(title, message, false,positive, (dialogInterface, i) -> {
            ...
            Intent intent = new Intent(BaseOfficeDetailActivity.this, LandingActivity.class);
    ...
    public void setOfficeDetailDialog(AlertDialog officeDetailDialog) {
        this.officeDetailDialog = officeDetailDialog;
    }

    public void validateCustomAlertDialog() {
        if (getOfficeDetailDialog() != null) {
            getOfficeDetailDialog().dismiss();
            doubleClick = false;
            controlDialogOfficeDetail = false;
        }
    }
    ...
    public void paintViewPointGeometryAsync(Envelope initialEnvelope, double paddingMap){
        try {
            mapView.setViewpointGeometryAsync(initialEnvelope, paddingMap);
        }catch(ArcGISRuntimeException e){
