...
import co.gov.and.coronapp.bluetrace.bluetooth.gatt.ACTION_DEVICE_PROCESSED
import co.gov.and.coronapp.bluetrace.bluetooth.gatt.CONNECTION_DATA
import co.gov.and.coronapp.bluetrace.bluetooth.gatt.DEVICE_ADDRESS
import co.gov.and.coronapp.bluetrace.logging.CentralLog
import co.gov.and.coronapp.bluetrace.protocol.BlueTrace
import co.gov.and.coronapp.bluetrace.services.BluetoothMonitoringService
import co.gov.and.coronapp.bluetrace.services.BluetoothMonitoringService.Companion.blacklistDuration
import co.gov.and.coronapp.bluetrace.services.BluetoothMonitoringService.Companion.maxQueueTime
import java.util.*
import java.util.concurrent.PriorityBlockingQueue

private const val TAG = "GattServer-StreetPassWorker"

class StreetPassWorker(val context: Context) {

    private val workQueue: PriorityBlockingQueue<Work> = PriorityBlockingQueue(5, Collections.reverseOrder())
    ...
    fun addWork(work: Work): Boolean {
        ...
        if (
                blacklist.any { it.uniqueIdentifier == work.device.address }
//                || blacklist.filter { it.uniqueIdentifier == work.connectable.manuData }.isNotEmpty()
    ...
    fun finishWork(work: Work) {
        ...
        if (work.isCriticalsCompleted()) {
            BluetraceUtils.broadcastDeviceProcessed(context, work.device.address)
...
//                        gatt.requestConnectionPriority(BluetoothGatt.CONNECTION_PRIORITY_HIGH)
        ...
        override fun onCharacteristicRead(
                    ...
                    if (BlueTrace.supportsCharUUID(characteristic.uuid)) {
                        ...
                        try {
                            ...
                            connectionRecord?.let {
                                BluetraceUtils.broadcastStreetPassReceived(
                                    context,
                                    connectionRecord
                                )
                            }
            ...
            if (BlueTrace.supportsCharUUID(characteristic.uuid)) {
                ...
                val deviceID = Preference.getDeviceID(context)
