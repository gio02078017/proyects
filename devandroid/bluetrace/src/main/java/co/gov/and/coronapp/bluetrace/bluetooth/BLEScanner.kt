package co.gov.and.coronapp.bluetrace.bluetooth
...
class BLEScanner constructor(context: Context, uuid: String, reportDelay: Long) {
    ...
    fun stopScan() {
        ...
        try {
            if (scanCallback != null && BluetraceUtils.isBluetoothAvailable()) { //fixed crash if BT if turned off, stop scan will crash.
